...
public function import(Request $request)
{
    ...
    foreach ($rows as $row) {
        ...
        $leadData = @array_combine($headers, $row);
        if ($leadData === false) { $skipped++; continue; }
        ...
        if (!empty($leadData['status'])) {
            switch (strtolower(trim($leadData['status']))) {
                case 'super lead': $leadData['status'] = 'Super Lead'; break;
                case 'deal':       $leadData['status'] = 'Deal'; break;
                case 'call back':  $leadData['status'] = 'Call Back'; break;
                case 'new lead':   $leadData['status'] = 'New Lead'; break;
                default:           $leadData['status'] = 'New Lead';
            }
        } else {
            $leadData['status'] = 'New Lead';
        }
        ...
        unset(
            $leadData['primary_number'], $leadData['alt_number_1'],
            $leadData['alt_number_2'], $leadData['alt_number_3'], $leadData['alt_number_4']
        );
